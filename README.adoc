...
- If your pod and his volume are still up, start by shuting down the pod (becareful to also stop the deployment/statefulset) and delete the volume using Longhorn UI.
- Go to the backup tab of Longhorn UI and restore the desired volume backup. You must check the _"Use Previous Name"_ checkbox in order to keep the old volume name.
- Next, go to the volume tab, select your newly restored volume and choose _"Create PV/PVC"_ option. Select _"Use Previous PVC"_ option and validate.
